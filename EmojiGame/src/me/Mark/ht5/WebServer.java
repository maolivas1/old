...
public class WebServer extends WebSocketServer {
	...
	public void onClose(WebSocket conn, int code, String reason, boolean remote) {
		if (Thingy.nick.containsKey(conn)) {
			Main.chat(Thingy.nick.get(conn) + " left");
			
			for (WebSocket con : Thingy.nick.keySet())
				con.send("leave " + Thingy.nick.get(conn));
		    Thingy.nick.remove(conn);
		    
		    if (Thingy.nick.size() < 3) {
		    	Thingy.restart();
		    	Thingy.sendAll("load");
		    }
		}
