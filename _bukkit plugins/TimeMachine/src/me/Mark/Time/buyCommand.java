...
public class buyCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (sender instanceof Player) {
			...
			if (args.length >= 1) {
			double price = invClick.getPrice(args[0].toUpperCase());
			if (price != -1.0) {
			price = price * count;
			if(economy != null) {
			...
			List<String> list = ConfigStuff.getWorths();
			...
			for (String block : list) {
				ItemStack sapling = new ItemStack(Material.getMaterial(block), 1);
				ItemMeta meta = sapling.getItemMeta();
				meta.setDisplayName(ChatColor.DARK_GREEN + "" + ChatColor.BOLD + block);
				ArrayList<String> lore = new ArrayList<String>();
				lore.add(ChatColor.GREEN + block + " for " + invClick.getPrice((block)));
				meta.setLore(lore);
