...
public class Fix implements Listener {
	...
	public void join(PlayerJoinEvent event) {
		...
		for (Player p : Bukkit.getOnlinePlayers())
        	...
        	TitleAPI.sendTabTitle(player, format("&9&lWelcome To &2&lMark's Wold"), null);
        	Commands.send(player, "&9&lWelcome To &2&lMark's Wold");
	}
	
	@EventHandler
	public void chat(PlayerChatEvent event) {
		event.setMessage(format(caps(event.getMessage())));
